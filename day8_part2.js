...
var image = height_array[0].map((x)=>x);
tracker_value = 2
for(var r =0 ;r<height;r++){
    for(var w=0;w<width;w++){
        for(var layer=1;layer<height_array.length;layer++){
            current_value = height_array[layer][r][w]
            tracker_value = height_array[0][r][w]
            if(current_value<tracker_value && tracker_value == 2){
                tracker_value = current_value;
                image[r][w] = tracker_value
                break
            }else {
                continue 
            }
